...
with tab1:
    ...
    col_state, col_county = st.columns([1, 2])

    with col_state:
        selected_state = st.selectbox("Select a state", options=states_sorted, index=states_sorted.index("TX") if "TX" in states_sorted else 0)

    state_df = df[df["state"] == selected_state].sort_values("name")
    county_options = state_df["name"].tolist()

    with col_county:
        selected_county = st.selectbox("Select a county", options=county_options, index=0)

    c = state_df[state_df["name"] == selected_county].iloc[0]
